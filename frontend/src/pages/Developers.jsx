...
import { Github, Linkedin, Database, Bug, Mail, Globe, Layers, Code2, Shield, Sparkles } from 'lucide-react';
...
const teamMembers = [
  ...
  },
  {
    ...
    skills: ['Automation', 'Python', 'Load Testing', 'Postman'],
    socials: { github: '#', linkedin: '#', portfolio: '#' },
  },
];
...
const mentor = {
  ...
  image: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Professor',
  description:
        ...
        <div className="wrapper relative z-10 text-center">
          ...
          <RevealSection delay={100}>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold font-display text-white mb-5 leading-tight">
              Meet Our{' '}
              <span className="text-gradient">Development Team</span>
            </h1>
          </RevealSection>
      ...
      <div className="wrapper">
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-14">
            {filters.map((f) => {
              ...
              return (
                ...
                >
                  <Icon size={15} className={active ? 'text-white' : 'text-brand-text-muted group-hover:text-brand-orange transition-colors'} />
                  {f}
                </button>
              );
            })}
              ...
              return (
                <RevealSection key={member.id} delay={idx * 80}>
                  <div className="group relative bg-brand-card rounded-2xl border border-brand-border overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:border-brand-orange/30 hover:shadow-[0_20px_60px_-15px_rgba(249,115,22,0.15)]">
                    {/* top gradient bar */}
                    <div className={`h-28 bg-gradient-to-br ${gradient} opacity-20 group-hover:opacity-30 transition-opacity duration-500`} />
                    ...
                    <div className="px-6 pb-8 text-center relative">
                      ...
                      <div className="relative -mt-14 mb-4 inline-block">
                        ...
                        <img
                          src={member.image}
                          alt={member.name}
                          className="relative w-28 h-28 rounded-full border-4 border-brand-card bg-brand-surface object-cover"
                        />
                        {/* role badges on avatar */}
                        <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 flex gap-1">
                          {member.category.map((cat) => {
                            const icons = { Frontend: Layers, Backend: Database, Testing: Bug };
                            const colors = { Frontend: 'bg-orange-500', Backend: 'bg-blue-500', Testing: 'bg-emerald-500' };
                            const CatIcon = icons[cat];
                            return (
                              <div key={cat} className={`p-1.5 ${colors[cat]} rounded-full text-white shadow-lg`}>
                                <CatIcon size={12} />
                              </div>
                            );
                          })}
                      ...
                      <p className="text-sm text-brand-text-secondary mb-4 font-medium">{member.role}</p>
                      ...
                      <div className="flex justify-center gap-3">
                        <a href={member.socials.github} className="p-2 rounded-lg bg-brand-surface text-brand-text-muted hover:text-white hover:bg-white/10 transition-all duration-200">
                          <Github size={17} />
                        </a>
                        <a href={member.socials.linkedin} className="p-2 rounded-lg bg-brand-surface text-brand-text-muted hover:text-blue-400 hover:bg-blue-500/10 transition-all duration-200">
                          <Linkedin size={17} />
                        </a>
                        <a href={member.socials.portfolio} className="p-2 rounded-lg bg-brand-surface text-brand-text-muted hover:text-brand-orange hover:bg-brand-orange/10 transition-all duration-200">
                          <Globe size={17} />
                        </a>
                      </div>
                    </div>
        ...
        {showMembers && filteredMembers.length === 0 && (
          <div className="text-center py-16">
            <p className="text-brand-text-secondary text-lg">No team members found in this category.</p>
          </div>
        )}
        ...
        {showMentor && (
          <RevealSection className="mb-20">
            <div className="relative rounded-3xl p-px bg-gradient-to-br from-brand-orange/40 via-brand-border to-blue-500/40 overflow-hidden group">
              ...
              <div className="absolute bottom-0 left-0 -ml-24 -mb-24 w-72 h-72 bg-blue-500/20 rounded-full blur-3xl opacity-30 animate-float pointer-events-none" style={{ animationDelay: '3s' }} />
              ...
              <div className="relative bg-brand-card rounded-[23px] p-8 md:p-12 z-10">
                <div className="flex flex-col md:flex-row items-center gap-10">
                  ...
                  <div className="text-center md:text-left flex-1">
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold font-display text-white mb-2">{mentor.name}</h2>
                    <p className="text-blue-400 text-lg mb-5 font-medium">{mentor.role}</p>
